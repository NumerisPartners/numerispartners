                ...
                <form action="{{ route('admin.blog.update', $blog) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="col-span-1 md:col-span-2">
                            ...
                            <x-text-input id="slug" class="block mt-1 w-full" type="text" name="slug" :value="old('slug', $blog->slug)" required />
                        ...
                        <div>
                            ...
                            @if($blog->featured_image)
                                <div class="mt-2">
                                    ...
                                    <img src="{{ asset('storage/' . $blog->featured_image) }}" alt="{{ $blog->title }}" class="mt-1 h-32 object-cover rounded">
                                </div>
                            @endif
                        </div>
                        ...
                        <div>
                            <div class="flex items-center">
                                <input id="is_published" name="is_published" type="checkbox" class="rounded dark:bg-gray-900 border-gray-300 dark:border-gray-700 text-indigo-600 shadow-sm focus:ring-indigo-500 dark:focus:ring-indigo-600 dark:focus:ring-offset-gray-800" {{ old('is_published', $blog->is_published) ? 'checked' : '' }}>
                                <x-input-label for="is_published" class="ml-2" value="{{ __('Publié') }}" />
                            </div>
                        ...
                        <div>
                            ...
                            <x-text-input id="published_at" class="block mt-1 w-full" type="datetime-local" name="published_at" :value="old('published_at', $blog->published_at ? $blog->published_at->format('Y-m-d\TH:i') : '')" />
                            <x-input-error for="published_at" class="mt-2" />
                            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Laisser vide pour publier immédiatement si "Publié" est coché.</p>
                        </div>
                    </div>
